...
function memegen_build_image( $args = array() ) {
	...
	//$sub_text = strtoupper( trim( $args['sub_text'] ) );
	//$bottom_text = strtoupper( trim( $args['bottom_text'] ) );

	//Texto Normal
	$top_text = trim( $args['top_text'] );
	$sub_text =  trim( $args['sub_text'] ) ;
	$bottom_text =  trim( $args['bottom_text']);
	...
	// sub layer text
	extract( memegen_font_size_guess( $textsize, ($width-$padding*2), $font, $sub_text, $fit ) );
	$from_side = ($width - $box_width)/2;
	$from_top = 520;
	// imagettftext( $im, $textsize, $angle, $from_side, $from_top, $textcolor, $font, $sub_text );
	memegen_imagettfstroketext( $im, $fontsize, $angle, $from_side, $from_top, $textcolorsub, $black, $font, $sub_text, 1 );
	...
	memegen_imagettfstroketext( $im, 40, $angle, $from_side, $from_top, $textcolor, $black, $font, $bottom_text, 1 );
