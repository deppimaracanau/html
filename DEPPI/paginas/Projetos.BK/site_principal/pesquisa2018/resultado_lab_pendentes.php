...
<html lang="pt-br">
    ...
    if(!@($conexao=pg_connect ("host=localhost dbname=pesquisa port=5432 user=postgres password=1"))) {
      echo "Não foi possível estabelecer uma conexão com o banco de dados.";
        	...
        	//TABELA: quantidades
     $consulta_quantidades_computacao = pg_query($conexao, "SELECT coordenador,laboratorio FROM laboratorios_restantes WHERE eixo = 'Computação';");
     ...
     while($row = pg_fetch_array($consulta_quantidades_computacao)) {
      ...
      $_SESSION['coordenador'.$i] = $row[0];
      $_SESSION['lab'.$i] = $row[1];
      $i++;
    }
    ...
    $consulta_quantidades_industria = pg_query($conexao, "SELECT coordenador,laboratorio FROM laboratorios_restantes WHERE eixo = 'Indústria';");
    ...
    while($row = pg_fetch_array($consulta_quantidades_industria)) {
      ...
      $_SESSION['coordenador1'.$i] = $row[0];
      $_SESSION['lab1'.$i] = $row[1];
      $i++;
    ...
    $consulta_quantidades_quimica_meio = pg_query($conexao, "SELECT coordenador,laboratorio FROM laboratorios_restantes WHERE eixo = 'Química e Meio Ambiente';");
    ...
    while($row = pg_fetch_array($consulta_quantidades_quimica_meio)) {
      ...
      $_SESSION['coordenador2'.$i] = $row[0];
      $_SESSION['lab2'.$i] = $row[1];
      $i++;
    }
    ...
    <div style="margin-top: 100px;">
      <?php 
          ...
          $qtd_linhas = $_SESSION["num_linhas_quantidades_computacao"];
          ...
          echo "<b>Laboratórios sem informações até o momento: </b><br><br>";
          ...
          for ($i=0; $i < $qtd_linhas ; $i++) { 
            ...
            echo $_SESSION['coordenador'.$i];

            echo html_table($data);
          }

          $qtd_linhas = $_SESSION["num_linhas_quantidades_industria"];
          ...
          for ($i=0; $i < $qtd_linhas ; $i++) { 
           ...
           echo "<br>";
           echo "<b> Laboratório: </b>";
           echo $_SESSION[ 'lab1'.$i];
           echo "<b> Coordenador: </b>";
           echo $_SESSION['coordenador1'.$i];

           echo html_table($data);
          }

          $qtd_linhas = $_SESSION["num_linhas_quantidades_quimica_meio"];
          ...
          for ($i=0; $i < $qtd_linhas ; $i++) { 
            ...
            echo "<br>";
            echo "<b> Laboratório: </b>";
            echo $_SESSION[ 'lab2'.$i];
            echo "<b> Coordenador: </b>";
            echo $_SESSION['coordenador2'.$i];
            echo html_table($data);
          }
          echo "</div>";
      ?>
