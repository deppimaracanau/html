...
class IndexController extends Zend_Controller_Action {
            ...
            if ($resultadoConsulta != null) {
                if ($resultadoConsulta['valido']) {
                    ...
                    $storage->write(array(
                        ...
                        "twitter" => $twitter,
                    	"tipoUsuario" => $result['id_tipo_usuario']//pegadondo o tipo do usuario(1,2 ou 3)
    ...
    public function recuperarSenhaAction() {
        ...
        if ($this->getRequest()->isPost() && $form->isValid($data)) {
            ...
            if (sizeof($resultado) > 0) {
                ...
                $config = new Zend_Config_Ini(APPLICATION_PATH . '/configs/application.ini', 'staging');
                $idEncontro = intval($config->encontro->codigo);

                $mail = new Application_Model_EmailConfirmacao();
                $mail->send($resultado[0]->id_pessoa, $idEncontro, Application_Model_EmailConfirmacao::MSG_RECUPERAR_SENHA);
                $this->_helper->flashMessenger->addMessage(
                        array('success' => 'E-mail enviado com sucesso, verifique seu e-mail.'));
            } else {
                $this->_helper->flashMessenger->addMessage(
                        array('alert' => 'Este e-mail não está cadastrado. '
                            . '<a href="' . $this->view->url(array(), "participar", true) . '">Cadastre-se</a>.'));
            }
        }
    }
    ...
    public function sobreAction() {
        ...
        $page = Zend_Controller_Front::getInstance()->getRequest()->getRequestUri();
        $dh = new Application_Model_Dh();
        $dh->updateCounter($page);
    }
